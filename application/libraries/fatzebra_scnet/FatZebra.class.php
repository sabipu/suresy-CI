...
//		public $sandbox_url = "https://gateway.fatzebra.com.au";
		...
		* @return \StdObject
		*/
		public function create_customer($first_name, $last_name, $reference, $email, $card_holder, $card_number, $card_expiry, $cvv, $ip_address, $extras=NULL) {
      ...
      if(!is_null($extras) && is_array($extras))
      {
		...
		*/
		...
		public function create_payment_plan($payment_method, $customer, $reference, $amount, $start_date, $frequency, $anniversary, $total_count, $total_amount, $currency="AUD", $setup_fee=0) {
			if(is_null($payment_method) || (strlen($payment_method) === 0)) throw new \InvalidArgumentException("payment_method is a required field.");
			if(is_null($customer) || (strlen($customer) === 0)) throw new \InvalidArgumentException("Customer is a required field.");
			if(is_null($reference) || (strlen($reference) === 0)) throw new \InvalidArgumentException("Reference is a required field.");
			if(is_null($amount) || ((int)$amount < 1)) throw new \InvalidArgumentException("Amount is invalid.");
			if(is_null($start_date) || (strlen($start_date) === 0)) throw new \InvalidArgumentException("Start Date is a required field.");
			if(is_null($frequency) || (strlen($frequency) === 0)) throw new \InvalidArgumentException("Frequency is a required field.");
			if(is_null($anniversary) || ((int)$amount < 1)) throw new \InvalidArgumentException("Anniversary is a required field.");
			if(is_null($total_count) || ((int)$total_count < 1)) throw new \InvalidArgumentException("Total Count is a required field.");
			if(is_null($total_amount) || ((int)$total_amount < 1)) throw new \InvalidArgumentException("Total Amount is a required field.");
			if(is_null($currency) || (strlen($currency) === 0)) throw new \InvalidArgumentException("Currency is a required field.");
			...
			return $this->do_request("POST", "/payment_plans", $payload);
		}
